...
function menorMayor(numeros) {
  ...
  // Tu código aca:
  var valorMa=0;
  var valorme=numeros[0];
  var k;
   k=numeros.length;

 
  for(i=0; i<k; i++){ 
    if(numeros[i]> valorMa){ 
      valorMa = numeros[i];
    }
     if (numeros[i]< valorme){ 
      valorme = numeros[i];
    }

  
  }
  var numeros =[valorme,valorMa]
...
function crearClasePersona() {
  class Persona {
    ...
    getPromedioEdad() {
      ...
      // Tu código aca:
      var n=this.amigos[0].edad
      for(var i=1; i<this.amigos.length; i++){
        
            
        n=n+this.amigos[i].edad;
      }
      return n/this.amigos.length;
    }
